...
namespace LaserGRBL
{
	...
	{
		public static event EventHandler ESPFileListChanged;
		...
		bool isFinishPrint = false;

		public double minX = 0, minY = 0, maxX = 0, maxY = 0;
		public bool IsBusy =>  isSendingFile || isFraming;

		public class ESPFileInfo
        {
			string mName;
			string mSize;
			public ESPFileInfo(string name, string size)
			{
		...
		public List<ESPFileInfo> mEspFileList;
		public List<ESPFileInfo> EspFileList()
		{
			if (mEspFileList == null)
				return new List<ESPFileInfo>();
			return mEspFileList;
		}
		public bool HasPrintSDCardFile = false;
		...
		private float mCurPrintProgress;
		private string mCurPrintName;
		...
		public void SetStatus(MacStatus newStatus)
		{
			lock (this)
			{
		...
		{
			ManageGenericMessage("Ready To Send File !");
			...
			string content = "; MKS Bounds: X" + left +" Y" + bottom+" to X" + right +" Y" + top + "\n";
			List<GrblCommand> list = file.fileList;
			double len = list.Count;
			double offetindex = 0;
			foreach (GrblCommand cmd in list)
			{
				if (offetindex%1000 == 0)
                {
					ManageGenericMessage("Loading File " + offetindex/len*100 + "% !");
				}
				offetindex++;
				content += cmd.Command + "\n";
			}
			if (content.Length > 0)
			{
				com.setCanRead(false);
				// 时间戳，用做boundary
				string timeStamp = DateTime.Now.Ticks.ToString("x");
				string url = "http://" + mCurIP + "/upload";
				...
				//文件
				/*FileStream fileStream = new FileStream("C:/Users/15954/Desktop/ABCD.png", FileMode.Open, FileAccess.Read);*/
				...
				string header = string.Format(dataFormat, "myfile[]", mCurFileName + ".nc");
				...
				try
				{
					...
					while (size > 0)
					{
						...
						double progress = ((double)offset / length) * 100;
						ManageGenericMessage("Sending File " + progress.ToString() + "% !");
					}
					...
					using (HttpWebResponse response = (HttpWebResponse)httpReq.GetResponse())
					{
						...
						Logger.LogMessage("-returnValue-------", "returnValue： {0}", returnValue);
						ManageGenericMessage("Send File Finish !");
						MessageBox.Show("Send File Finish Named " + mCurFileName + ".NC", "Success!");
						response.Close();
						readStream.Close();
				...
				{
					ManageGenericMessage("Send File Fail !");
					MessageBox.Show("Send File Fail!", Strings.WarnMessageBoxHeader);
		...
		public void SendProgram(string filename, string ip)
		{
			if (HasProgram)
			{
				if (filename != null)
				{
					//去掉中文等特殊字符
					mCurFileName = Regex.Replace(filename, @"[^a-zA-Z0-9]", "");
					mCurIP = ip;
					isSendingFile = true;
					SendFileThread = new Thread(StartSendFile);
		...
		public void SaveProgram(System.Windows.Forms.Form parent, bool header, bool footer, bool between, int cycles)
		{
			if (HasProgram)
			{
				...
				if (filename != null)
				{
					...
					decimal bottom = LoadedFile != null && LoadedFile.Range.DrawingRange.ValidRange ? LoadedFile.Range.DrawingRange.Y.Min : 0;
					string content = "; MKS Bounds: X" + left + " Y" + bottom + " to X" + right + " Y" + top + "\n";
					file.SaveProgram(filename, header, footer, between, cycles, content);
				}
			}
		}
        ...
        {
			if (com.IsOpen && mMachineStatus == MacStatus.Idle)
            {
                if (mEspFileList != null)
                {
					mEspFileList = null;
                }
                mQueue.Enqueue(new GrblCommand("[ESP210]\n"));
			}
        }

		private bool changePrintingSDCardFile = false;
		public void PrintSDCardFile(string filename)
		{
			if (com.IsOpen && mMachineStatus == MacStatus.Idle)
			{
				changePrintingSDCardFile = true;
				mQueue.Enqueue(new GrblCommand("[ESP220]/"+filename+"\n"));
			}
		}
		...
		public void DeleteSDCardFile(string filename)
		{
				...
				{
					mEspFileList = null;
					mQueue.Enqueue(new GrblCommand("[ESP215]/" + filename + "\n"));
					mQueue.Enqueue(new GrblCommand("[ESP210]\n"));
				}
			}
				catch (Exception ex)
				{
					Logger.LogException("DeleteSDCardFile", ex);
				}
		}

		public void MKSFrame(string filename)
		{
			try
			{
				if (com.IsOpen && mMachineStatus == MacStatus.Idle)
				{
					if (HasPrintSDCardFile)
                    {
						mCurPrintName = filename;
						if (minX == 0 && minY == 0 && maxX == 0 && maxY == 0)
						{
							// 查询文件bounds\
							isFraming = true;
                            mQueue.Enqueue(new GrblCommand(("[ESP801]/" + filename + "\n"), 0, true));
							ManageGenericMessage("Reading file ...");
							/*  byte b = GetImmediate(("[ESP801]/" + filename + "\n"));
							  SendImmediate(b);*/
							/*			com.setCanRead(false);*/
						} else
                        {
							string minxstr = minX.ToString();
							string minystr = minY.ToString();
							string maxxstr = maxX.ToString();
							string maxystr = maxY.ToString();
							mQueue.Enqueue(new GrblCommand("G0 X" + minxstr + " Y" + minystr + " \r\n"));
							mQueue.Enqueue(new GrblCommand("M3 S5 F1000\r\n"));
							mQueue.Enqueue(new GrblCommand("G1 Y" + maxystr + " \r\n"));
							mQueue.Enqueue(new GrblCommand("G1 X" + maxxstr + " \r\n"));
							mQueue.Enqueue(new GrblCommand("G1 Y" + minystr + " \r\n"));
							mQueue.Enqueue(new GrblCommand("G1 X" + minxstr + " \r\n"));
							mQueue.Enqueue(new GrblCommand("M5\r\n"));
							mQueue.Enqueue(new GrblCommand("G0 X0 Y0\r\n"));
						}
					} else
                    {
						ExecuteCustombutton("G0 X[left] Y[bottom]\r\nM3 S[$30 * 3 / 100] F1000\r\nG1 Y[top]\r\nG1 X[right]\r\nG1 Y[bottom]\r\nG1 X[left]\r\nM5\r\nG0 X0 Y0\r\n");
                    }
				}
			}
			catch (Exception ex)
			{
				Logger.LogException("MKSFrame", ex);
			}
		}
		...
		public void Configure(ComWrapper.WrapperType wraptype, params object[] conf)
		{
			...
			else if (wraptype == ComWrapper.WrapperType.MKSLaserESP32 && (com == null || com.GetType() != typeof(ComWrapper.MKSLaserESP32)))
				com = new ComWrapper.MKSLaserESP32();
		...
		public bool IsConnected => com.IsOpen && mMachineStatus != MacStatus.Disconnected && mMachineStatus != MacStatus.Connecting;
        ...
        public bool JogEnabled
		{
			get
			{
				if (SupportTrueJogging)
					return IsConnected && (MachineStatus == GrblCore.MacStatus.Idle || MachineStatus == GrblCore.MacStatus.Jog) && !IsBusy;
				else
					return IsConnected && (MachineStatus == GrblCore.MacStatus.Idle || MachineStatus == GrblCore.MacStatus.Run) && !InProgram && !IsBusy;
			}
		}
		...
		private bool CanSend()
		{
			...
			return next != null && HasSpaceInBuffer(next) && !isSendingFile;
		}
		...
		{
			//Logger.LogMessage("ManageReceivedLine------------------", rline);
			if (IsCommandReplyMessage(rline))
			{
				if (isFraming && rline.ToLower().StartsWith("error"))
				{
					isFraming = false;
					com.setCanRead(true);
				}
			...
			else if (IsESPSDCardFileMessage(rline))
				ManageESPSDCardFileMessage(rline);
			else if (IsESPNoSDCardMessage(rline))
				ManageESPNoSDCardMessage(rline);
			else if (IsESPBoundsMessage(rline))
				ManageESPBoundsMessage(rline);
			else
				ManageGenericMessage(rline);
			if (HasPrintSDCardFile && changePrintingSDCardFile && (MachineStatus == MacStatus.Run || MachineStatus == MacStatus.Hold))
            {
				changePrintingSDCardFile = false;
				ManageBrokenOkMessage(rline);
			}
			if (HasPrintSDCardFile && IsFinishSDCardPrintMessage(rline))
            {
				SetIsFinishPrint(true);
				//ManageFinishSDCardPrintMessage(rline);
			}
		}
		...
		private bool IsESPSDCardFileMessage(string rline) => rline.StartsWith("[FILE") && rline.EndsWith("]");
		private bool IsESPNoSDCardMessage(string rline) => rline.StartsWith("No SD Card");
		private bool IsESPBoundsMessage(string rline) => rline.ToLower().StartsWith("bounds");
		private bool IsFinishSDCardPrintMessage(string rline) => rline.ToLower().Contains("print finish");

		private void	SetIsFinishPrint(bool isFinish)
        {
			isFinishPrint = isFinish;
		}

		private void ManageFinishSDCardPrintMessage()
		{
			SetIsFinishPrint(false);
			try
			{
				if (mLoopCount > 1)
                {
					...
					{
						changePrintingSDCardFile = true;
						mQueue.Enqueue(new GrblCommand("[ESP220]/" + mCurPrintName + "\n"));
					}
				}
			}
			catch (Exception ex)
			{
				Logger.LogMessage("ManageFinishSDCardPrintMessage", "Ex on [{0}] message", "except");
				Logger.LogException("ManageFinishSDCardPrintMessage", ex);
			}
		}

		private void ManageESPNoSDCardMessage(string rline)
		{
			try
			{
				mEspFileList = null;
				ESPFileListChanged?.Invoke(this, null);
				ManageGenericMessage(rline);
			}
			catch (Exception ex)
			{
				Logger.LogMessage("ManageESPSDCardFileMessage", "Ex on [{0}] message", rline);
				Logger.LogException("ManageESPSDCardFileMessage", ex);
			}
		}
		private void ManageESPBoundsMessage(string rline)
		{
			try
			{
				string sline = rline.Substring(7).Replace("(", "").Replace(")", "");
				string[] arr = sline.Split(",".ToCharArray());
				if (arr.Length > 3)
                {
					minX = Convert.ToDouble(arr[0]);
					minY = Convert.ToDouble(arr[1]);
					maxX = Convert.ToDouble(arr[2]);
					maxY = Convert.ToDouble(arr[3]);
				}
				isFraming = false;
				/*ClearQueue(true);*/
				MKSFrame(mCurPrintName);
				com.setCanRead(true);
				ManageGenericMessage(rline);
			}
			catch (Exception ex)
		...
		private void ManageESPSDCardFileMessage(string rline)
		{
			try 
			{
				string sline = rline.Substring(7).Replace("]", "");
				string[] arr = sline.Split("|".ToCharArray()); 
				if (arr.Length > 1)
                {
					string name = arr[0];
					if (!name.Contains("/") && (name.ToLower().Contains(".nc") || name.ToLower().Contains(".gc") || name.ToLower().Contains(".gcode")))
                    {
						string size = arr[1];
						if (!size.Contains("-1"))
                        {
							long sizes = long.Parse(size.Substring(5));
							string value = GetSizeString(sizes);

							ESPFileInfo info = new ESPFileInfo(name, value);
							if (mEspFileList == null)
                            {
								mEspFileList = new List<ESPFileInfo>();
								mEspFileList.Add(info);
                            } else if (!mEspFileList.Contains(info))
                            {
								mEspFileList.Add(info);
							}
						}
		...
		private void ParseSD(string p)
		{
			//SD: 26.33,/ DLC32001W.NC
			string sfs = p.Substring(3, p.Length - 3);
			string[] fs = sfs.Split(",".ToCharArray());
			string sname = fs[1].Substring(1, fs[1].Length - 1);
			SetSD(ParseFloat(fs, 0), sname);
		}
		public float CurrentProgress { get { return mCurPrintProgress; } }
		public string CurrentPrintName { get { return mCurPrintName; } }
		protected void SetSD(float p, string n)
		{
			mCurPrintProgress = p;
			mCurPrintName = n;
		}
		...
		protected virtual void ParseMachineStatus(string data)
		{
			...
			if (isFinishPrint && var == MacStatus.Idle)
            {
				ManageFinishSDCardPrintMessage();
			}
		}
		...
		public bool CanReOpenFile
		{
			get
			{
				...
				return CanLoadNewFile && lastFile != null && System.IO.File.Exists(lastFile) && !IsBusy;
			}
		}

		public bool CanLoadNewFile
		{ get { return !InProgram && !IsBusy; } }
		public bool CanGetSDCardFile
		{ get { return IsConnected && !IsBusy && !(MachineStatus == MacStatus.Run || MachineStatus == MacStatus.Hold || MachineStatus == MacStatus.Cooling); } }

		public bool CanSendFile
		{ get { return IsConnected && (HasProgram || HasPrintSDCardFile) && IdleOrCheck && !IsBusy; } }
		public bool CanMKSSendFile
		{ get { return IsConnected && HasProgram && IdleOrCheck && !IsBusy; } }

		public bool CanAbortProgram
		{ get { return IsConnected && (HasProgram || HasPrintSDCardFile) && !isSendingFile && (MachineStatus == MacStatus.Run || MachineStatus == MacStatus.Hold || MachineStatus == MacStatus.Cooling); } }

		public bool CanImportExport
		{ get { return IsConnected && MachineStatus == MacStatus.Idle; } }

		public bool CanResetGrbl
		{ get { return IsConnected && MachineStatus != MacStatus.Disconnected && !IsBusy; } }

		public bool CanSendManualCommand
		{ get { return IsConnected && MachineStatus != MacStatus.Disconnected && !InProgram && !IsBusy; } }

		public bool CanDoHoming
		{ get { return IsConnected && (MachineStatus == MacStatus.Idle || MachineStatus == GrblCore.MacStatus.Alarm) && Configuration.HomingEnabled && !IsBusy; } }

		public bool CanDoZeroing
		{ get { return IsConnected && MachineStatus == MacStatus.Idle && WorkPosition != GPoint.Zero && !IsBusy; } }

		public bool CanUnlock
		{ get { return IsConnected && (MachineStatus == MacStatus.Idle || MachineStatus == GrblCore.MacStatus.Alarm) && !IsBusy; } }

		public bool CanFeedHold
		{ get { return IsConnected && MachineStatus == MacStatus.Run && !IsBusy; } }
		...
		{ get { return IsConnected && !InProgram && (MachineStatus == MacStatus.Idle || MachineStatus == MacStatus.Alarm) && !IsBusy; } }
